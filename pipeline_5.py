...
# descriptions='combine the all stats'      
@component(base_image=BASE_IMAGE)    
def combine_stats(
    json_stat_list: list, #Input[Dataset],
    all_stats: Output[Dataset],
):
    import pandas as pd
    import json
    from utils.augment_price_data import augment_price_data, obtain_syn_data, labeling_function, tech_indicator_calc       
    ...
    stat_list = []
    for json_stat in json_stat_list:
        with open(json_stat, 'r') as file:
            stat = json.load(file)
        stat_list.append(pd.DataFrame(stat))
    all_stats_df = pd.concat(stat_list, ignore_index=True)
    all_stats_df.to_csv(all_stats.path, index=False)        
...
def pipeline(
    ...
    # combine all the stats, which are jsons
    outputs_list = [get_origint_data_stats_op.outputs['df_stats']]
    for i in range(len(get_data_stats_op_list)):
        output = get_data_stats_op_list[i].outputs['df_stats']
        outputs_list.append(output)
    
    combine_stats_op = combine_stats(json_stat_list=outputs_list)
    combine_stats_op.set_cpu_limit('1').set_memory_limit('4G')
